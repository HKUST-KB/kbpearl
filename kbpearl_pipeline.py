...
class KBPearl():
    ...
    def senmantic_graph_constrcution(self, single_text, time_list, noun_phrase, relation_phrase, official_named_entity_mention_dict, last_entity_dict):
        ...
        for noun, noun_backup in noun_phrase.items():
            ...
            '''Pipeline result from the previous text!!!!'''
            if noun in last_entity_dict.keys() and noun_backup in last_entity_dict.keys():
                print('old nouns....'+noun)

                if noun in last_entity_dict.keys():
                    target_entity_list = last_entity_dict[noun]
                elif noun_backup in last_entity_dict.keys():
                    target_entity_list = last_entity_dict[noun_backup]
                if target_entity_list != [] and target_entity_list!= ['new entity']:
                    entity_id = target_entity_list[0]
                    entity_content = self.obtain_details_from_wikidata(entity_id)
                    candidate_entities_details.update({entity_id:entity_content['nb_sitelinks'][0]})
                    old_noun_list.append(noun)
                    entity_content_dict.update({entity_id:entity_content})
                    print('old thing!', noun, entity_content)
                    noun_entity_dict.update({noun:target_entity_list})

            else:
                '''Not detected in the pipeline results from the previous text'''
        ...
        print('time!!!')
        print(elapsed_time)
        return (noun_entity_dict, relation_predicate_dict, old_noun_list, elapsed_time)
    ...
    def my_kbpearl(self, id, single_text, open_ie_result, last_entity_dict):
        ...
        #print('-' * 30)
        #for triple in triple_list_with_candidate_named_entity_mentions:
            #pprint(triple)
        #print('-' * 30)
        ...
        (noun_entity_dict, relation_predicate_dict, old_noun_list,elapsed_time) = self.senmantic_graph_constrcution(single_text, time_list, noun_phrase, relation_phrase, official_named_entity_mention_dict, last_entity_dict)
    ...
    def main_pipeline(self,dataset, openie_tool):
        ...
        result_file = '../data/kbpearl_output_new/'+dataset+'_'+openie_tool+'_pipeline.json'
        ...
        for id, content in gt.items():
            ...
            source_corpus_list = sen_tokenizer.tokenize(content['text'])
            total_time = 0
            for i in range(0, len(source_corpus_list)):

                print(source_corpus_list[i])
                print({"0":open_ie_result[id][str(i)]})
                (noun_entity_dict, relation_predicate_dict, old_noun_list,elapsed_time) = self.my_kbpearl(id, source_corpus_list[i], {"0":open_ie_result[id][str(i)]}, last_entity_dict)
                total_time+=elapsed_time
                print('#'*40)
                print(last_entity_dict)
                print(noun_entity_dict)
                print(relation_predicate_dict)
                print('#'*40)
                temp_dict = {}
                for noun, entity_list in noun_entity_dict.items():
                    if noun not in last_entity_dict.keys():
                        temp_dict.update({noun:entity_list})
                    if noun in last_entity_dict.keys() and noun in old_noun_list:
                        temp_dict.update({noun:entity_list})
                last_entity_dict = temp_dict
...
if __name__ == '__main__':
    ...
    dataset = 'nytimes'
    openie_tool = 'minieS'
    pearl.main_pipeline(dataset, openie_tool)
