...
class KBPearl():
    ...
    def obtain_candidate_predicates(self, relation, question_word, single_text):
        if relation == '':
            return []
        if relation in AUXILLIARYY_VERBS:
            return []
        relation = re.sub(re.compile('#|@|-|~|\$|\^|\+|\&'),'', relation)
        if question_word not in ['how','How','when','When','where','Where'] and relation.find(' ')==-1:
            new_relation = relation+'*'
        elif question_word in ['which', 'Which'] and relation.find(' ') == -1:
            for time in ['year', 'month', 'date']:
                if time in single_text.split(' '):
    ...
    def get_official_noun_phrases_and_relation_phrases(self, single_text, official_named_entity_mention_dict, time_list,
                                    triple_list_with_candidate_named_entity_mentions, source_corpus_list):
        print(source_corpus_list)
        print(len(source_corpus_list))
        noun_phrase = {}
        relation_phrase = {}
        sen_noun_index_dict = {}
        sen_relation_index_dict = {}
        count = -1
        for triple_info in triple_list_with_candidate_named_entity_mentions:
            ...
            for triple, triple_result in triple_info.items():
                ...
                if sub not in noun_phrase.keys() and sub not in QUESTIONS:
                    if single_text[-1:] == '?':
                        ...
                        temp_noun_list.append(' '.join(final_list))
                    else:
                        noun_phrase.update({sub: sub_backup})
                        temp_noun_list.append(sub)
                else:
                    temp_noun_list.append(sub)
                ...
                temp_relation_list.append(re.lower())
            ...
            print(temp_relation_list)
            sen_noun_index_dict.update({count:temp_noun_list})
            sen_relation_index_dict.update({count:temp_relation_list})
        ...
        for mention, type in official_named_entity_mention_dict.items():
            ...
            for i in range(0, len(source_corpus_list)):
                print(i)
                if source_corpus_list[i].find(mention) != -1:
                    print(i, 'found!!')
                    if i in sen_noun_index_dict.keys():
                        temp_list = sen_noun_index_dict[i]
                        if mention not in temp_list:
                            temp_list.append(mention)
                            sen_noun_index_dict.update({i: temp_list})
                            print('found _1')
                            print({i: temp_list})
                    else:
                        sen_noun_index_dict.update({i: [mention]})
                        print('found _2')
                        print({i: [mention]})
        ...
        return(noun_phrase, relation_phrase, sen_noun_index_dict, sen_relation_index_dict)
    ...
    def min_distance_of_list(self, list_a, list_b, d):
        for a in list_a:
            for b in list_b:
                if abs(a-b)<=d:
                    return True
    ...
    def senmantic_graph_constrcution(self, single_text, time_list, noun_phrase, relation_phrase, official_named_entity_mention_dict,final_noun_loc_dict, sen_noun_index_dict,sen_relation_index_dict):
        ...
        for noun, noun_backup in noun_phrase.items():
            ...
            for entity in candidate_entities_details.keys():
                ...
                g.add_edge(noun, entity, weight/5.0)
        ...
        for relation, relation_backup in relation_phrase.items():
            print(relation, relation_backup)
            ...
            if single_text[-1:]=='?':
                ...
                for word in QUESTIONS:
                    if single_text.find(word)!=-1:
                        query_list = self.obtain_candidate_predicates(relation_backup, word, single_text)
                        question_produce = True
                if question_produce == False:
                    query_list = self.obtain_candidate_predicates(relation_backup, '', single_text)
            else:
                query_list = self.obtain_candidate_predicates(relation_backup, '', single_text)
            ...
            for en1 in candidate_entity_list:
                for en2 in candidate_entity_list:
                    ...
                    entity_content_dict[en1]) not in entity_compare_list:
                        if entity_noun_dict[en1] in final_noun_loc_dict.keys() and entity_noun_dict[en2] in final_noun_loc_dict.keys():
                            en_distance = abs(final_noun_loc_dict[entity_noun_dict[en1]]-final_noun_loc_dict[entity_noun_dict[en2]])
                            if en_distance>MAX_ENTITYY_DISTANCE:
                                continue
                        else:
                            if not self.min_distance_of_list(sen_noun_index_dict[entity_noun_dict[en1]],sen_noun_index_dict[entity_noun_dict[en2]],1):
                                #print(en1, en2, 'Too far away')
                                continue
                        entity_compare_list.append(
                            (en1, en2, 'entity', 'entity', entity_content_dict[en1], entity_content_dict[en2]))
                        print(entity_noun_dict[en1],entity_noun_dict[en2])
            ...
            for p1 in candidate_predicate_list:
                for p2 in candidate_predicate_list:
                    ...
                    predicate_content_dict[p1]) not in predicate_compare_list:
                        # print(p1,p2)
                        if self.min_distance_of_list(sen_relation_index_dict[predicate_relation_dict[p1]],sen_relation_index_dict[predicate_relation_dict[p2]],0):
        ...
        for en in candidate_entity_list:
            for p in candidate_predicate_list:
                if (en, p, 'entity', 'predicate') not in entity_predicate_compare_list and self.min_distance_of_list(sen_noun_index_dict[entity_noun_dict[en]],sen_relation_index_dict[predicate_relation_dict[p]],0):
    ...
    def find_word_sen_index_dict(self, word_phrase_list, sentence_list,triple_list_with_candidate_named_entity_mentions):
        noun_phrase = word_phrase_list
        source_corpus_list = sentence_list
        sen_noun_index_dict = {}
        ...
        for noun in noun_phrase:
            ...
            for i in range(0, len(source_corpus_list)):
                temp_list = []
                for key, value in triple_list_with_candidate_named_entity_mentions[i].items():
                    for item in value:
                        temp_list.append(list(item.keys())[0])
                if noun != '':
                    if source_corpus_list[i].find(noun) != -1:
                        noun_sentence_index.append(i)
                    else:
                        if noun in temp_list:
                            noun_sentence_index.append(i)
            if noun != '':
                sen_noun_index_dict.update({noun: noun_sentence_index})
                print(noun, noun_sentence_index)
        return sen_noun_index_dict

    def dict_inverse(self,dict):
        new_dict ={}
        for key, value_list in dict.items():
            for item in list(set(value_list)):
                if item not in new_dict.keys():
                    ...
                    temp_list = new_dict[item]
                    temp_list.append(key)
                    new_dict.update({item: temp_list})
        return new_dict
    ...
    def my_kbpearl(self, id, single_text, open_ie_result, result_file):
        ...
        for i in range(0, len(triple_list_with_candidate_named_entity_mentions)):
            print(i)
            print(triple_list_with_candidate_named_entity_mentions[i])
        print('-' * 30)
        ...
        source_corpus_list = sen_tokenizer.tokenize(single_text)
        (noun_phrase, relation_phrase, sen_noun_index_dict, sen_relation_index_dict) = self.get_official_noun_phrases_and_relation_phrases(single_text, official_named_entity_mention_dict, time_list, triple_list_with_candidate_named_entity_mentions, source_corpus_list)
        print(noun_phrase)
        print(relation_phrase)

        '''Sentence index dict'''
        sen_noun_index_dict = self.dict_inverse(sen_noun_index_dict)
        sen_relation_index_dict = self.dict_inverse(sen_relation_index_dict)
        pprint(sen_noun_index_dict)
        pprint(sen_relation_index_dict)

        '''Document index dict'''
        #print(noun_phrase)
        noun_index_dict = {}
        for noun in noun_phrase:
            location = single_text.find(noun)
            #print(noun,location)
            if location!=-1 and noun!='':
                noun_index_dict.update({noun:location})
        final_noun_loc_dict = {}
        count = 0
        for noun in sorted(noun_index_dict, key=noun_index_dict.get):
            #print(noun, count)
            final_noun_loc_dict.update({noun:count})
            count+=1

        '''find candidate entities for the set of noun phrases (also find the predicates of these entities)'''
        (noun_entity_dict, relation_predicate_dict) = self.senmantic_graph_constrcution(single_text, time_list, noun_phrase, relation_phrase, official_named_entity_mention_dict, final_noun_loc_dict, sen_noun_index_dict,sen_relation_index_dict)
        print(noun_entity_dict)
        print(relation_predicate_dict)
    ...
    def main(self,dataset, openie_tool, mode):
        ...
        result_file = '../data/kbpearl_output_new/'+dataset+'_'+openie_tool+mode+'.json'
...
if __name__ == '__main__':
    ...
    openie_tool = 'minieS'
    mode = '_nn_'
    pearl.main(dataset, openie_tool, mode)
