...
class IndexingProfile(object):
    ...
    def entity_to_document(self, item, type_matcher):
        ...
        # Fetch aliases
        aliases_temp = item.get_all_terms()
        aliases_temp.remove(enlabel)
        aliases = []
        for item_temp in aliases_temp:
            aliases.append(self.transfer_relation_phrase_to_norm_pattern(item_temp))
        ...
        for extractor in self.alias_properties:
            extra_aliases_temp += extractor.extract(item)
        extra_aliases = []
        for item_temp in extra_aliases_temp:
            extra_aliases.append(self.transfer_relation_phrase_to_norm_pattern(item_temp))
